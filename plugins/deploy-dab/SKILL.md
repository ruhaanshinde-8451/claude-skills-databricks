---
name: deploy-dab
description: Deploy a Databricks Asset Bundle to dev and return structured deployment output.
argument-hint: --repo-path <absolute-or-relative-path> --target dev
---

# /deploy-dab

Deploy a Databricks Asset Bundle to the **dev** environment from a specified repository path.

## Required Inputs
- `--repo-path <path>`: Path to the Databricks Asset Bundle repository root (must contain `databricks.yml` or `databricks.yaml`).
- `--target <env>`: Deployment target. This skill supports `dev` only.
...
2. If `target` is not `dev`, stop with a structured failure (`unsupported_target`).
...
1. Authentication failure  
   ...
   - `message=Databricks authentication failed for target dev. Check CLI login/profile configuration and retry.`
...
3. Other deploy failures  
   ...
   - `message=Databricks bundle deploy failed for target dev. Review stderr for details.`

## Manual Verification (Story requirement)
Run this skill end-to-end against the representative dev DAB from the scaffold spike and confirm:
