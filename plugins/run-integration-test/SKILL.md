...
argument-hint: --repo-path <path> (--job-id <id> | --job-name <name>) [--target dev] [--profile <profile>] [--poll-interval-sec 30] [--timeout-sec 1800] [--baseline-duration-sec <seconds>]
---
...
- `--profile <profile>`: Databricks CLI profile to use. Defaults to match `target`.
...
```bash
/run-integration-test --repo-path /path/to/dab-repo --job-id 123456789 --target dev --profile dev
```
...
3. Resolve `profile`: default to match `target` if not provided.
4. Resolve job identifier:
   - If `--job-id` provided, use it directly.
   - If `--job-name` provided, resolve with Databricks CLI and require exactly one match.
5. Config drift check — compare deployed job config against `databricks.yml`:
```bash
   databricks bundle validate --target <target> -p <profile> -o json
   databricks jobs get --job-id <job_id> -p <profile> -o json
```
...
6. Trigger job run:
```bash
   databricks jobs run-now --job-id <job_id> -p <profile> -o json
```
   Extract `run_id`.
7. Poll run state until terminal state or timeout:
```bash
   databricks runs get --run-id <run_id> -p <profile> -o json
```
...
8. Build run URL using workspace host and run ID:
   - `https://<workspace-host>#job/<job_id>/run/<run_id>`
9. If run fails, extract task-level error message from run output:
   - Prefer task-level output from:
```bash
     databricks jobs get-run-output --run-id <task_run_id> -p <profile> -o json
```
   - Fallback to parent run output:
```bash
     databricks jobs get-run-output --run-id <run_id> -p <profile> -o json
```
